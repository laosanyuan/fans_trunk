...
class ViewFleetStrategy(BaseButtonStrategy):
    ...
    def _get_fleet_buttons(self) -> list:
        ...
        results = []
        for item in fleets:
            channels, _ = self._channel_data_provider.get_fleet_summary(item.id)
            results.append([InlineKeyboardButton(f'🚗 {item.name} ({channels})', callback_data=f'{self.tag}#fleet%{item.id}')])
