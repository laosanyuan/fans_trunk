...
class ChannelDataProvider:
    ...
    def get_fleet_summary(self, fleet_id: int) -> tuple[int, int]:
        ...
        fake_members = 0
        for item in self._fake_users:
            if item.score >= fleet.min_score and item.score < fleet.max_score:
                fake_channels += 1
                fake_members += item.member_count
    ...
    def _load_fake_users(self) -> None:
        """从配置文件加载广告数据"""
        try:
            with open(self._fake_users_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                for item in data:
                    tmp = ChannelDTO(-1, item['user_name'], item['title'], -1, -1, True, True,
                                     item['score'], datetime.now(), False, item['member_count'])
                    self._fake_users.append(tmp)
        except Exception as e:
            print(f"加载假用户配置失败: {str(e)}")
            self._fake_users = []
